...
def portfolio_from_csv():
    ...
    for key, value in portfolios.items():
        ...
        #create portfolio
        try:
            req  = investment_portfolio.Create_Portfolio(my_portfolio)
        except:
        ...
        try:
            for h in range(0,len(value),500):
                hldgs = value[h:h+500]
                req  = investment_portfolio.Create_Portfolio_Holdings(str(key),hldgs)
        except:
...
def get_unit_test_portfolios():
    ...
    '''
    portfolio_names = []
    res = investment_portfolio.Get_Portfolios_by_Selector('type','unit test portfolio')
    try:
...
def get_unit_test_delete():
    ...
    '''
    portfolios = investment_portfolio.Get_Portfolios_by_Selector('type','unit test portfolio')['portfolios']
    print(portfolios)
    for p in portfolios:
        holdings = investment_portfolio.Get_Portfolio_Holdings(p['name'],False)
        # delete all holdings
        for h in holdings['holdings']:
            timestamp = h['timestamp']
            rev = h['_rev']
            investment_portfolio.Delete_Portfolio_Holdings(p['name'],timestamp,rev)
        investment_portfolio.Delete_Portfolio(p['name'],p['timestamp'],p['_rev'])
...
def compute_unit_tests():
    ...
    for p in portfolios:
        ...
        holdings = investment_portfolio.Get_Portfolio_Holdings(p,False)['holdings']
        ...
        for ph in range(0,len(holdings)):
            ...
            for i in range(0,len(instruments),500):
                ...
                ia = instrument_analytics.Compute_InstrumentAnalytics(ids,analytics)
                #for j in results...
                if 'error' not in ia:
                    for j in ia:
                        r = {
